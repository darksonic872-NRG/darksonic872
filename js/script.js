...
document.addEventListener('DOMContentLoaded',()=>{const rows=[...document.querySelectorAll('.treasure-row[data-image], .object-screen-btn[data-image]')];if(!rows.length)return;const box=document.createElement('div');box.className='guide-lightbox';box.innerHTML='<button type="button" aria-label="Fermer">×</button><div class="guide-lightbox-panel"><img alt="Capture d’emplacement"><p class="guide-lightbox-caption"></p></div>';document.body.appendChild(box);const img=box.querySelector('img');const caption=box.querySelector('.guide-lightbox-caption');const close=()=>box.classList.remove('open');box.querySelector('button').addEventListener('click',close);box.addEventListener('click',e=>{if(e.target===box)close()});document.addEventListener('keydown',e=>{if(e.key==='Escape')close()});rows.forEach(r=>r.addEventListener('click',()=>{img.src=r.dataset.image;caption.textContent=((r.closest('.object-step')?.querySelector('h4')?.textContent)||(r.querySelector('span')?.textContent)||'Capture').trim();box.classList.add('open')}));});

// V4.3 - cases objets intégrées au cheminement
document.addEventListener('DOMContentLoaded',()=>{const key='legaia_guide_objects_v1';let saved=[];try{saved=JSON.parse(localStorage.getItem(key)||'[]')}catch(e){}const set=new Set(saved.map(String));const boxes=[...document.querySelectorAll('input[data-guide-object]')];boxes.forEach(b=>{b.checked=set.has(String(b.dataset.guideObject));b.addEventListener('change',()=>{localStorage.setItem(key,JSON.stringify(boxes.filter(x=>x.checked).map(x=>String(x.dataset.guideObject))));});});});
